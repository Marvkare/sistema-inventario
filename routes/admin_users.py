...
def list_roles():
    ...
    try:
        ...
        # ✅ CAMBIO: Apunta a la nueva plantilla unificada
...
def edit_role(role_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            cursor.execute("UPDATE role SET name = %s, description = %s WHERE id = %s", (name, description, role_id))
            conn.commit()
            
            log_activity(action="Edición de Rol", category="Roles", resource_id=role_id, details=f"Se editó el rol: {name}")
            flash('Rol actualizado exitosamente.', 'success')
            return redirect(url_for('admin_users.list_roles'))

        # Lógica GET para obtener los datos del rol
        cursor.execute("SELECT id, name, description FROM role WHERE id = %s", (role_id,))
        role = cursor.fetchone()
        if not role:
            flash("Rol no encontrado.", "danger")
            return redirect(url_for('admin_users.list_roles'))
        
        # ✅ CAMBIO: Renderiza la plantilla unificada, pasando el 'role' para llenar el formulario
        return render_template('admin/admin_roles.html', role=role)
...
def list_users():
    ...
    try:
        ...
        cursor.execute("""
            ...
            GROUP BY u.id, u.username, u.nombres, u.telefono
            ORDER BY u.username
        """)
...
def create_user():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            cursor.execute(
                "INSERT INTO user (username, password_hash, nombres, telefono) VALUES (%s, %s, %s, %s)",
                (username, hashed_password, nombres, telefono)
            )
            ...
            conn.commit()
            log_activity(action="Creación de Usuario", category="Usuarios", details=f"Se creó el usuario: {username} ({nombres})")
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if new_password:
                ...
                cursor.execute(
                    "UPDATE user SET username = %s, password_hash = %s, nombres = %s, telefono = %s WHERE id = %s",
                    (username, hashed_password, nombres, telefono, user_id)
                )
            else:
                cursor.execute(
                    "UPDATE user SET username = %s, nombres = %s, telefono = %s WHERE id = %s",
                    (username, nombres, telefono, user_id)
                )
        ...
        cursor.execute("SELECT id, username, nombres, telefono FROM user WHERE id = %s", (user_id,))
        ...
        # ✅ CAMBIO: Renderiza la nueva plantilla unificada
        return render_template('admin/admin_users.html', user=user, all_roles=all_roles, user_role_ids=user_role_ids)
...
def manage_permissions():
    ...
    try:
        ...
        # 3. Lógica GET (con la modificación)
        
        # ✅ CAMBIO 1: Obtener el ID del rol desde los argumentos de la URL
        ...
        for row in cursor.fetchall():
            ...
            role_perms_map.setdefault(str(role_id), []).append(permission_id)

        return render_template(
            'admin/manage_permissions.html', 
            all_roles=all_roles, 
            all_permissions=all_permissions, 
            role_perms_map=role_perms_map,
            # ✅ CAMBIO 2: Pasar el ID seleccionado a la plantilla
...
def reset_password_request(user_id):
    ...
    try:
        ...
        log_activity(action="Solicitud de reseteo de contraseña", category="Usuarios", resource_id=user_id, details="Se generó un enlace de reseteo") # 'details' añadido para claridad
...
def reset_password(token):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            log_activity(action="Contraseña restablecida", category="Usuarios", resource_id=user['id'], details="El usuario restableció su contraseña") # 'details' añadido para claridad
